...
const defaultSqonDeletionHandler = ({ indexToRemove, dependentIndices, internalStateContainer: s }) =>
...
const AdvancedSqonBuilder = (props) => {
	const {
		...
		fieldDisplayNameMap = {},
		ButtonComponent = ({ className, ...rest }) => <button className={`button ${className}`} {...rest} />,
		getSqonDeleteConfirmation = defaultSqonDeletionHandler,
		apiFetcher = defaultApiFetcher,
		referenceColors = ['#cbeefb', '#fce8d3', '#eed5e9', '#cbebf1', '#f9d3d4', '#d5d7e9', '#fad9ea', '#f3ebd0'],
	...
	const getColorForReference = (referenceIndex) => referenceColors[referenceIndex % referenceColors.length];
